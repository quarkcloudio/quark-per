...
class ResourceUpdateController extends Controller
{
    ...
    public function handle($resource, Request $request)
    {
        ...
        $validator = $getCalledClass::validatorForUpdate($request, new $getCalledClass);
        ...
        $data = $this->getSubmitData(
            (new $getCalledClass)->updateFields($request),
            (new $getCalledClass)->beforeSaving($request, $request->all()) // 保存前回调
        );

        $updateResult = $getCalledClass::newModel()->where('id', $request->id)->update($data);

        if($updateResult) {
            $model = $getCalledClass::newModel()->where('id', $request->id)->first();
        } else {
            $model = $updateResult;
        }
    ...
    protected function getSubmitData($fields, $submitData)
    {
        ...
        foreach ($fields as $value) {
            if(isset($submitData[$value->name])) {
                ...
                if($value->type === 'password' && !empty($submitData[$value->name])) {
